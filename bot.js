...
client.on("messageCreate", async (message) => {
  ...
  } else if (command === "!races" || "!tr") {
    const username = message.content.slice(3);
    const totalRacesToday = await getTotalRacesToday(username);
    if (totalRacesToday) {
      message.reply(`${username}'s total races todau is: ${totalRacesToday}.`);
    } else {
      message.reply(`Could not retrieve ${username}'s total races today.`);
    }
  } else {
...
async function getHighestWpmToday(username) {
  try {
    ...
    const todayRaces = races.filter(
      (race) => new Date(race.time * 1000) > last24hours
    );
